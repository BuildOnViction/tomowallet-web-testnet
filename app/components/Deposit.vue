  ...
  <div class="transfer">
    <div v-if="showSummary && !error" class="transfer-summary">
      <div>
        ...
        <br />Click
        <a target="_blank" :href="`${explorer}/txs/${hash}`">here</a> to check your transaction on TomoScan
      </div>
      <button
        class="btn-big btn-black outline mt30"
        @click="doAnotherTransaction"
      >Do another transaction</button>
    </div>
    <div v-else>
      ...
      <div>
        ...
        <div
          :style="{opacity: errorAmount ? 1 : 0}"
          class="transfer-error text-right mt15"
        >{{errorAmount || '&nbsp;'}}</div>
      </div>
      <div
        :style="{opacity: error ? 1 : 0}"
        class="transfer-error text-center mt15"
      >{{error || '&nbsp;'}}</div>
      <div class="text-center">
        <button v-if="!isSending" class="btn-big btn-black mt30" @click="deposit">
          <span class="ml30">SEND</span>
          <fa icon="arrow-right" class="fs15 ml30" />
        </button>
        <button v-else class="btn-big btn-black mt30">SENDING...</button>
      </div>
    </div>
  </div>
</template>
